...
void ST_Drawer (boolean fullscreen, boolean refresh)
{
  ...
  
    {
        ...
        // 체력 표시
       snprintf(line1, sizeof(line1), "HP:%3d", players[0].health);
        // 이벤트 메시지 (plyr->message 가 NULL 아닐 때)
        if (players[0].message)
            snprintf(line2, sizeof(line2), "%.*s", 16, players[0].message);
        lcdtextwrite(line1, NULL, 1);
	lcdtextwrite(NULL, line2, 2);
    }

        // (2) FND: 앞3자리=아머%, 뒤3자리=준비 탄약 수
    {
        int armor = players[0].armorpoints;
        if (armor < 0) armor = 0;
        if (armor > 100) armor = 100;
        int ammoidx = weaponinfo[players[0].readyweapon].ammo;
        int ammo    = (ammoidx>=0)? players[0].ammo[ammoidx]: 0;
        if (ammo < 0) ammo = 0;
        if (ammo > 999) ammo = 999;
        int value = armor * 1000 + ammo;
        fndDisp(value, 0);
    }

    // (3) 8-LED: 준비된 탄약 퍼센트 비례해 LED 개수 결정
    {
        int ammoidx = weaponinfo[players[0].readyweapon].ammo;
        int ammo    = (ammoidx>=0)? players[0].ammo[ammoidx]: 0;
        int maxammo = (ammoidx>=0)? players[0].maxammo[ammoidx]: 1;
        int pct     = (maxammo>0)? (ammo*100)/maxammo : 0;
    ...
    // (4) 컬러 PWM LED: 체력 %
    {
        int pct = (players[0].health * 100) / 100; // MAXHEALTH=100
        if (pct<0) pct=0; if (pct>100) pct=100;
        pwmSetPercent(pct, PWM_COLOR_R);
    }
